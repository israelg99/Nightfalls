...
public class WorldSlotsScreen implements Screen {
	...
	public WorldSlotsScreen(boolean l) {
		load = l;
	}
	...
	public void show() {
		...
		TextButton slot1 = new TextButton("Slot 1", blueButton);
		slot1.addListener(new ClickListener() {

			@Override
			public void clicked(InputEvent event, float x, float y) {
				MainMenuSFX.playClick();
				table.addAction(sequence(moveTo(0, -stage.getHeight(), .5f), run(new Runnable() {

					@Override
					public void run() {
						((Game) Gdx.app.getApplicationListener()).setScreen(new GameScreen(load, 1));
						MainMenuMusic.mmStop();
					}
				})));
			}
		});
		
		TextButton slot2 = new TextButton("Slot 2", blueButton);
		slot2.addListener(new ClickListener() {
			...
			public void clicked(InputEvent event, float x, float y) {
				...
				table.addAction(sequence(moveTo(0, -stage.getHeight(), .5f), run(new Runnable() {
					...
					@Override
					public void run() {
						((Game) Gdx.app.getApplicationListener()).setScreen(new GameScreen(load, 2));
						MainMenuMusic.mmStop();
					}
				})));
			}
		});
		TextButton slot3 = new TextButton("Slot 3", blueButton);
		slot3.addListener(new ClickListener() {
			...
			public void clicked(InputEvent event, float x, float y) {
				...
				table.addAction(sequence(moveTo(0, -stage.getHeight(), .5f), run(new Runnable() {
					...
					@Override
					public void run() {
						((Game) Gdx.app.getApplicationListener()).setScreen(new GameScreen(load, 3));
						MainMenuMusic.mmStop();
					}
				})));
			}
		...
		table.add(slot1).spaceBottom(15).row();
		table.add(slot2).spaceBottom(15).row();
		table.add(slot3).spaceBottom(15).row();
		...
		Timeline.createSequence().beginSequence()
				.push(Tween.set(slot1, ActorAccessor.ALPHA).target(0))
				.push(Tween.set(slot2, ActorAccessor.ALPHA).target(0))
				.push(Tween.set(slot3, ActorAccessor.ALPHA).target(0))
				.push(Tween.set(buttonBack, ActorAccessor.ALPHA).target(0))
				.push(Tween.from(gameLogo, ActorAccessor.ALPHA, .25f).target(0))
				.push(Tween.to(slot1, ActorAccessor.ALPHA, .25f).target(1))
				.push(Tween.to(slot2, ActorAccessor.ALPHA, .25f).target(1))
				.push(Tween.to(slot3, ActorAccessor.ALPHA, .25f).target(1))
