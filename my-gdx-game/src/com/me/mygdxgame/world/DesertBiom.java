...
public class DesertBiom {
	...
	public static void makeHill(List<Block> map, int width, int x, int y, int h, int rHillStone, int rHillDirt) {
		for(int i = 0; i <= h; i++) {
			for(int j = 0; j <= i; j++) {
				map.set((y - h + i) * width + x + j, BiomUtils.chooseRare(rHillStone) && j < i ? new Block(2) : BiomUtils.chooseRare(rHillDirt) ? new Block(6) : new Block(12));
				map.set((y - h + i) * width + x - j, BiomUtils.chooseRare(rHillStone) && j < i ? new Block(2) : BiomUtils.chooseRare(rHillDirt) ? new Block(6) : new Block(12));
			}
		}
	}
	...
	public static void makeLeaf(List<Block> map, int width, int x, int y, int h) {
		for(int i = 0; i <= h; i++) {
			for(int j = 0; j <= i; j++) {
				map.set((y - h + i) * width + x + j, map.get((y - h + i) * width + x + j).ID != 7 ? new Block(13) : map.get((y - h + i) * width + x + j));
				map.set((y - h + i) * width + x - j, map.get((y - h + i) * width + x - j).ID != 7 ? new Block(13) : map.get((y - h + i) * width + x - j));
			}
		}
	}
